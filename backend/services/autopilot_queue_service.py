...
class AutoPilotQueueService:
    ...
    async def run_queue(self, queue_id: str) -> None:
        ...
        try:
            ...
            queue.last_error = "Internal queue error — check server logs"
    ...
    async def wait_for_reset(
        ...
        This is a best-effort timer.  If ``reset_time_str`` is provided
        (e.g. "3:00 PM"), the service will attempt to parse it and sleep
        ...
        if reset_time_str:
            try:
                # Parse the reset time relative to today
                from datetime import datetime as dt
                for fmt in ("%I:%M %p", "%I:%M%p", "%I %p", "%I%p"):
                    try:
                        reset_dt = dt.strptime(reset_time_str.strip(), fmt)
                        now = dt.utcnow()
                        target = now.replace(
                            hour=reset_dt.hour,
                            minute=reset_dt.minute,
                            second=0,
                            microsecond=0,
                        )
                        if target <= now:
                        ...
                        sleep_seconds = max(int((target - now).total_seconds()), 60)
                        break
                    except ValueError:
                        continue
            except Exception:
                self._logger.warning(
                    "Could not parse reset time '%s', defaulting to 1 hour",
                    reset_time_str,
                )

        self._logger.info(
            "Queue %s waiting for reset — auto-resume in %d seconds",
            queue_id, sleep_seconds,
        )

        await asyncio.sleep(sleep_seconds)

        # Auto-resume if the queue is still in WAITING_RESET
        if queue.status == QueueStatus.WAITING_RESET:
            self.resume_queue(queue_id)
            self._logger.info(
                "Queue %s auto-resumed after token reset wait", queue_id
            )
